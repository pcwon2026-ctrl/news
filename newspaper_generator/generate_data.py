...
INDEX_HTML_FILE = os.path.join(NEWSPAPER_FOLDER, "index.html")
...
# ============================================================
# 1면 index.html 카톡 미리보기 사진 (og:image) 자동 갱신
# ============================================================
import re

def update_index_og_image(articles_data):
    """1면 index.html의 og:image 태그를 매일 톱기사 사진으로 자동 갱신.
    카톡에 1면 URL을 올렸을 때 미리보기 사진이 뜨게 하는 역할.
    """
    print("\n" + "=" * 60)
    print("[3.4/4] 1면 카톡 미리보기 사진(og:image) 갱신")
    print("=" * 60)

    if not os.path.exists(INDEX_HTML_FILE):
        print(f"[X] index.html 없음: {INDEX_HTML_FILE}")
        return

    # 톱기사 선정: 사진 있는 기사 중 날짜가 가장 최근인 것
    articles = articles_data.get("articles", [])
    candidates = []
    for a in articles:
        # photoFile(Base64 추출 결과) 또는 일반 photo 경로 둘 다 후보
        if a.get("photoFile"):
            candidates.append(a)
        elif a.get("photo") and not a.get("photo", "").startswith("data:"):
            candidates.append(a)

    if not candidates:
        print("[!] 사진 있는 기사가 없음. og:image 업데이트 건너뜀.")
        return
    ...
    photo_path = top.get("photoFile") or top.get("photo", "")
    if photo_path.startswith("http"):
        og_image_url = photo_path
    else:
        og_image_url = f"{SITE_URL}/{photo_path.lstrip('/')}"

    print(f"   톱기사: [{top.get('dept','?')}] {top.get('headline','')[:50]}")
    ...
    new_tw_tag = f'<meta name="twitter:image" content="{og_image_url}">'

    # og:image 태그 처리 (있으면 교체, 없으면 og:url 다음에 추가)
    og_image_pattern = re.compile(r'<meta\s+property=["\']og:image["\'][^>]*?>')
    if og_image_pattern.search(html):
        html = og_image_pattern.sub(new_og_tag, html)
    else:
        # og:url 다음 줄에 삽입
        html = re.sub(
    ...
    # twitter:image 태그 처리
    tw_image_pattern = re.compile(r'<meta\s+name=["\']twitter:image["\'][^>]*?>')
    if tw_image_pattern.search(html):
        html = tw_image_pattern.sub(new_tw_tag, html)
    else:
        # twitter:card 다음 줄에 삽입
        html = re.sub(
...
if __name__ == "__main__":
    ...
    if save_file(js_text):
        ...
        update_index_og_image(articles_data)
